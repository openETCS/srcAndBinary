...
void InformationFilter_init_Informat(outC_InformationFilter_Informat *outC)
{
  ...
  outC->_L108.BG_Common_Header.m_version = M_VERSION_Previous_versions_acc;
  ...
  outC->_L108.BG_Common_Header.BG_centerDetectionInaccuraccura.nominal = 0;
  outC->_L108.BG_Common_Header.BG_centerDetectionInaccuraccura.d_min = 0;
  outC->_L108.BG_Common_Header.BG_centerDetectionInaccuraccura.d_max = 0;
  outC->_L108.BG_Common_Header.q_nvlocacc = 0;
  outC->_L108.BG_Common_Header.noCoordinateSystemHasBeenAssign = kcg_true;
  ...
  outC->_L108.Radio_Common_Header.m_ack = M_ACK_No_acknowledgement_requir;
  ...
  outC->_L108.Radio_Common_Header.m_version = M_VERSION_Previous_versions_acc;
  for (i = 0; i < 30; i++) {
  ...
  outC->bufferMsg.BG_Common_Header.m_version = M_VERSION_Previous_versions_acc;
  ...
  outC->bufferMsg.BG_Common_Header.bgPosition.motionDirection =
    unknownDirection_Obu_BasicTypes;
  outC->bufferMsg.BG_Common_Header.BG_centerDetectionInaccuraccura.nominal = 0;
  outC->bufferMsg.BG_Common_Header.BG_centerDetectionInaccuraccura.d_min = 0;
  outC->bufferMsg.BG_Common_Header.BG_centerDetectionInaccuraccura.d_max = 0;
  outC->bufferMsg.BG_Common_Header.q_nvlocacc = 0;
  outC->bufferMsg.BG_Common_Header.noCoordinateSystemHasBeenAssign = kcg_true;
  ...
  outC->bufferMsg.Radio_Common_Header.m_ack = M_ACK_No_acknowledgement_requir;
  ...
  /* 1 */ flagsForLevelChange_init_Inform(&outC->_1_Context_1);
  /* 1 */ TransitionBuffer_init_Informati(&outC->Context_1);
}
...
{
  outC->init = kcg_true;
  /* 1 */ flagsForLevelChange_reset_Infor(&outC->_1_Context_1);
  /* 1 */ TransitionBuffer_reset_Informat(&outC->Context_1);
}
  ...
  outC_InformationFilter_Informat *outC)
{
  ...
  static _191_SSM_ST_SM1 SM1_state_sel;
  /* InformationFilter_Pkg::InformationFilter::SM1 */
  static _191_SSM_ST_SM1 SM1_state_act;
  /* InformationFilter_Pkg::InformationFilter::_L99 */
  static ReceivedMessage_T_Common_Types_ _L99;
  ...
  /* last_init_ck_storeInBuffer */ if (outC->init) {
    ...
    kcg_copy_ReceivedMessage_T_Comm(&_L99, &outC->bufferMsg);
    tmp = outC->storeInBuffer;
  }
  /* sel_SM1 */ switch (SM1_state_sel) {
    case SSM_st_State1_SM1 :
      SM1_state_act = _190_SSM_st_State2_SM1;
      break;
    case _190_SSM_st_State2_SM1 :
  ...
  /* 4 */ if (_L99.valid & tmp) {
    kcg_copy_ReceivedMessage_T_Comm(&tmp1, &_L99);
  }
  else {
    kcg_copy_ReceivedMessage_T_Comm(
      &tmp1,
  ...
  /* act_SM1 */ switch (SM1_state_act) {
    ...
    case _190_SSM_st_State2_SM1 :
      ...
      outC->SM1_state_nxt = _190_SSM_st_State2_SM1;
      break;
    case SSM_st_State1_SM1 :
  ...
  TransitionBuffer_InformationFil(
    ...
    (kcg_bool)
      ((*inMorcStatus).session.phase == sp_terminated_RCM_Session_Types),
    &outC->Context_1);
  outC->currBuffersize = outC->Context_1.outStacksize;
  /* 1 */ if ((*inMessage).valid & outC->bufferHasPriority) {
    kcg_copy_ReceivedMessage_T_Comm(&outC->_L108, inMessage);
  }
  ...
  /* 3 */ if (outC->bufferHasPriority) {
    kcg_copy_ReceivedMessage_T_Comm(&_L99, &outC->Context_1.Out);
  }
  else /* 5 */ if (!kcg_comp_ReceivedMessage_T_Comm(
      &outC->_L108,
      (struct_102028 *) &cDefaultRM_Common_Types_Pkg)) {
    kcg_copy_ReceivedMessage_T_Comm(&_L99, &outC->_L108);
  }
  else {
    kcg_copy_ReceivedMessage_T_Comm(&_L99, inMessage);
  }
  /* 1 */
  flagsForLevelChange_Information(
    inModeLevel,
    inMessage,
    inFilterEvents,
    &outC->_1_Context_1);
  kcg_copy_T_Data_From_Track_MASS(&outC->IF_toML, &outC->_1_Context_1.IF_toML);
  /* 2 */ if (_L99.valid) {
    /* FilterSequence */
    FilterSequence_InformationFilte(
      &_L99,
      (*inModeLevel).level,
    ...
    kcg_copy_ReceivedMessage_T_Comm(
      &outC->bufferMsg,
      (ReceivedMessage_T_Common_Types_ *) &cDefaultRM_Common_Types_Pkg);
    kcg_copy_ReceivedMessage_T_Comm(&outC->outMessage, &_L99);
  }
