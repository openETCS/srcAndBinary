...
void RBC__ProcessConditionalEm_init_(outC_RBC__ProcessConditionalEm_ *outC)
{
  static kcg_int i1;
  static kcg_int i;
  ...
  outC->session.m_TrainData.m_OnboardPhoneNumbers[0].valid = kcg_true;
  for (i = 0; i < 15; i++) {
    outC->session.m_TrainData.m_OnboardPhoneNumbers[0].telephoneNumber[i] = 0;
  }
  outC->session.m_TrainData.t_train_ref = 0;
  outC->session.m_TrainData.nc_cdtrain = NC_CDTRAIN_Cant_Deficiency_80_m;
  outC->session.m_TrainData.nc_train = NC_TRAIN_Train_does_not_belong_;
  outC->session.m_TrainData.l_train = 0;
  outC->session.m_TrainData.v_maxtrain = 0;
  outC->session.m_TrainData.m_loadinggauge = M_LOADINGGAUGE_The_train_does_n;
  ...
  for (i = 0; i < 4; i++) {
    outC->session.m_TrainData.m_TractionIdentities[i].m_voltage =
      M_VOLTAGE_Line_not_fitted_with_;
    outC->session.m_TrainData.m_TractionIdentities[i].nid_ctraction = 0;
  }
  for (i = 0; i < 5; i++) {
    outC->session.m_TrainData.m_NationalSystemIdentities[i] = 0;
  }
  ...
  outC->session.m_PosData.q_length = Q_LENGTH_No_train_integrity_inf;
  ...
  outC->CONDITIONAL_EMERGENCY_STOP_SM_s = SSM_st_IDLE_CONDITIONAL_EMERGEN;
...
void RBC__ProcessConditionalEm_reset(outC_RBC__ProcessConditionalEm_ *outC)
{
  outC->init = kcg_true;
}

/* RBC_Model_Pkg::RBC__ProcessConditionalEmergencyStop */
void RBC__ProcessConditionalEm_RBC_M(
  ...
  outC_RBC__ProcessConditionalEm_ *outC)
{
  ...
  static kcg_bool IfBlock1_clock_CONDITIONAL_EMER;
  ...
  static SSM_ST_CONDITIONAL_EMERGENCY_ST CONDITIONAL_EMERGENCY_STOP_SM_s;
  /* RBC_Model_Pkg::RBC__ProcessConditionalEmergencyStop::CONDITIONAL_EMERGENCY_STOP_SM */
  static SSM_ST_CONDITIONAL_EMERGENCY_ST _5_CONDITIONAL_EMERGENCY_STOP_S;
  ...
  radioTrackTrainMessageId = /* 2 */ _100_RadioTrackTrainHeader__Get(&tmp3);
  /* 4 */ RadioTrainTrackMessage__GetHead(inRadioTrainTrackMessage, &tmp4);
  radioTrainTrackMessageId = /* 2 */ _148_RadioTrainTrackHeader__Get(&tmp4);
  /* last_init_ck_radioTrackTrainMessageWasSentEmergencyStopId */ if (outC->init) {
    IfBlock1_clock_CONDITIONAL_EMER = kcg_false;
    last_invalidAck = kcg_false;
    CONDITIONAL_EMERGENCY_STOP_SM_s = SSM_st_IDLE_CONDITIONAL_EMERGEN;
    outC->init = kcg_false;
    last_radioTrackTrainMessageWasS = NID_EM_DEFAULT;
  }
  else {
    last_invalidAck = outC->invalidAck;
    CONDITIONAL_EMERGENCY_STOP_SM_s = outC->CONDITIONAL_EMERGENCY_STOP_SM_s;
    IfBlock1_clock_CONDITIONAL_EMER = outC->validAck;
    last_radioTrackTrainMessageWasS = outC->radioTrackTrainMessageWasSentEm;
  }
  /* sel_CONDITIONAL_EMERGENCY_STOP_SM */ switch (CONDITIONAL_EMERGENCY_STOP_SM_s) {
    case SSM_st_IDLE_CONDITIONAL_EMERGEN :
      if (radioTrackTrainMessageId == 15) {
        _5_CONDITIONAL_EMERGENCY_STOP_S = SSM_st_SEND_CONDITIONAL_EMERGEN;
      }
      else {
        _5_CONDITIONAL_EMERGENCY_STOP_S = SSM_st_IDLE_CONDITIONAL_EMERGEN;
      }
      break;
    case SSM_st_SEND_CONDITIONAL_EMERGEN :
      if (radioTrainTrackMessageId == 147) {
        _5_CONDITIONAL_EMERGENCY_STOP_S = _54_SSM_st_TRY_PROCESS_ACKNOWLE;
      }
      else if (radioTrainTrackMessageId != 147) {
        _5_CONDITIONAL_EMERGENCY_STOP_S = _55_SSM_st_AWAIT_ACKNOWLEDGEMEN;
      }
      else {
        _5_CONDITIONAL_EMERGENCY_STOP_S = SSM_st_SEND_CONDITIONAL_EMERGEN;
      }
      break;
    case _55_SSM_st_AWAIT_ACKNOWLEDGEMEN :
      if (radioTrainTrackMessageId == 147) {
        _5_CONDITIONAL_EMERGENCY_STOP_S = _54_SSM_st_TRY_PROCESS_ACKNOWLE;
      }
      else {
      ...
      break;
    case _54_SSM_st_TRY_PROCESS_ACKNOWLE :
      if (IfBlock1_clock_CONDITIONAL_EMER & (radioTrackTrainMessageId == 15)) {
        _5_CONDITIONAL_EMERGENCY_STOP_S = SSM_st_SEND_CONDITIONAL_EMERGEN;
      }
      else if (IfBlock1_clock_CONDITIONAL_EMER) {
        _5_CONDITIONAL_EMERGENCY_STOP_S = SSM_st_IDLE_CONDITIONAL_EMERGEN;
      }
  ...
  /* act_CONDITIONAL_EMERGENCY_STOP_SM */ switch (_5_CONDITIONAL_EMERGENCY_STOP_S) {
    case _54_SSM_st_TRY_PROCESS_ACKNOWLE :
      /* 3 */ RadioTrainTrackMessage__GetHead(inRadioTrainTrackMessage, &tmp4);
      IfBlock1_clock_CONDITIONAL_EMER = /* 1 */
        RadioTrainTrackHeader__Get_NID_(&tmp4) ==
        last_radioTrackTrainMessageWasS;
      outC->radioTrackTrainMessageWasSentEm = last_radioTrackTrainMessageWasS;
      /* ck_IfBlock1 */ if (IfBlock1_clock_CONDITIONAL_EMER) {
        outC->validAck = kcg_true;
        outC->invalidAck = kcg_false;
        /* 5 */ SessionManagement__GetPosData_R(&(*inDataBus).session, &tmp1);
        /* 5 */
        PosData__Update_RBC_Session_Pkg(&tmp1, inRadioTrainTrackMessage, &tmp2);
        /* 5 */
      ...
      outC->CONDITIONAL_EMERGENCY_STOP_SM_s = _54_SSM_st_TRY_PROCESS_ACKNOWLE;
    ...
    case _55_SSM_st_AWAIT_ACKNOWLEDGEMEN :
      ...
      outC->CONDITIONAL_EMERGENCY_STOP_SM_s = _55_SSM_st_AWAIT_ACKNOWLEDGEMEN;
      kcg_copy__9_RadioTrackTrainMess(
        &outC->outDataBus.messageQueue,
        &(*inDataBus).messageQueue);
      break;
    case SSM_st_SEND_CONDITIONAL_EMERGEN :
      ...
      kcg_copy_SessionManagement_T(&outC->session, &(*inDataBus).session);
      outC->CONDITIONAL_EMERGENCY_STOP_SM_s = SSM_st_SEND_CONDITIONAL_EMERGEN;
      /* 3 */
...
/* $**************** KCG Version 6.4 (build i21) ****************
** RBC__ProcessConditionalEm_RBC_M.c
** Generation date: 2015-11-10T22:49:43
*************************************************************$ */
