...
void Send_P045_reset_MessageLibBaseline2(
  outC_Send_P045_MessageLibBaseline2 *outC)
{
  /* 1 */ Write_P045_reset_TM_trackside(&outC->Context_1);
}
  ...
  outC_Send_P045_MessageLibBaseline2 *outC)
{
  ...
  kcg_copy_P045_trackside_int_T_TM(&outC->_L4, P045);
  outC->_L5 = outC->_L4.valid;
  outC->tmp = outC->_L5;
  kcg_copy_CompressedPackets_T_Common_Types_Pkg(&outC->_L3, B_data_in);
  if (outC->tmp) {
    /* 1 */ Write_P045_TM_trackside(&outC->_L4, &outC->_L3, &outC->Context_1);
