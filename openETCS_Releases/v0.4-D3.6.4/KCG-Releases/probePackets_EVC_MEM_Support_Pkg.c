  ...
  outC_probePackets_EVC_MEM_Support_Pkg *outC)
{
  ...
  for (i = 0; i < 7; i++) {
    ...
    outC->p27.diffArray[i].q_diff = Q_DIFF_Cant_Deficiency_specific_category;
    outC->p27.diffArray[i].nc_cddiff =
  ...
  outC_probePackets_EVC_MEM_Support_Pkg *outC)
{
  ...
  /* 1 */
  Read_P027V1_Legacy_for_ML_TM_specific(&(*actualMessage).packets, &outC->p27);
