...
void MRSP_limit_to_EOA_TA_Export(
  ...
  /* TA_Export::MRSP_limit_to_EOA::EoA */ L_internal_Type_Obu_BasicTypes_ EoA,
  ...
  /* TA_Export::MRSP_limit_to_EOA::limited */ MRSP_section_t_TrackAtlasTypes *limited)
{
  ...
  *cont = kcg_true;
  if ((0 <= i) & (i < 110)) {
    kcg_copy_MRSP_section_t_TrackAt(&tmp, &(*MRSP)[i]);
  }
  else {
  ...
  /* 1 */ if (tmp.Loc_Abs > EoA) {
    kcg_copy_MRSP_section_t_TrackAt(&MRSP_section0, &tmp);
  ...
  /* 3 */ if (tmp.Loc_Abs == END_OF_SSP_encoding_TA_Export) {
    ...
    kcg_copy_MRSP_section_t_TrackAt(limited, &MRSP_section0);
