...
void TimingChecker_init_CheckEurorad(outC_TimingChecker_CheckEurorad *outC)
{
...
void TimingChecker_reset_CheckEurora(outC_TimingChecker_CheckEurorad *outC)
{
  outC->init = kcg_true;
}
...
void TimingChecker_CheckEuroradioMes(
  ...
  outC_TimingChecker_CheckEurorad *outC)
{
  ...
  static kcg_bool _1_connectingStatusConnecting_I;
  /* CheckEuroradioMessage::TimingChecker::IfBlock1::else::then::connectingStatusConnecting */
  static kcg_bool connectingStatusConnecting_IfBl;
  ...
  TimestampKnown = (*rtmMessage).Radio_Common_Header.t_train !=
    cTTrainUnknown1_CheckEuroradioM;
  /* last_init_ck_ignoreTimestamp */ if (outC->init) {
    last_lastKnownTTrain = cTTrainUnknown1_CheckEuroradioM;
    outC->init = kcg_false;
  ...
  /* ck_IfBlock1 */ if (IfBlock1_clock) {
    ...
    /* ck_anon_activ */ if (else_clock_IfBlock1) {
      connectingStatusConnecting_IfBl = (*radioStatus).connection.status ==
        mcs_connected_RCM_Types_Pkg;
      /* ck_connectingStatusConnecting */ if (connectingStatusConnecting_IfBl) {
        outC->otherTimingError = kcg_false;
      ...
      _1_connectingStatusConnecting_I = (*radioStatus).connection.status ==
        mcs_connected_RCM_Types_Pkg;
      /* ck_connectingStatusConnecting */ if (_1_connectingStatusConnecting_I) {
        outC->otherTimingError = kcg_false;
        outC->sequenceError = kcg_false;
        outC->ignoreTimestamp = kcg_true;
        outC->lastKnownTTrain = 0;
...
/* $**************** KCG Version 6.4 (build i21) ****************
** TimingChecker_CheckEuroradioMes.c
** Generation date: 2015-12-03T13:39:52
*************************************************************$ */
