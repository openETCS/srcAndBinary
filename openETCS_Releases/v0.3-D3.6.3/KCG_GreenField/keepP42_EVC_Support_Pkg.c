...
void keepP42_init_EVC_Support_Pkg(outC_keepP42_EVC_Support_Pkg *outC)
{
  ...
  outC->store.BG_Common_Header.m_version = M_VERSION_Previous_versions_acc;
  ...
  outC->store.BG_Common_Header.BG_centerDetectionInaccuraccura.nominal = 0;
  outC->store.BG_Common_Header.BG_centerDetectionInaccuraccura.d_min = 0;
  outC->store.BG_Common_Header.BG_centerDetectionInaccuraccura.d_max = 0;
  outC->store.BG_Common_Header.q_nvlocacc = 0;
  outC->store.BG_Common_Header.noCoordinateSystemHasBeenAssign = kcg_true;
  ...
  outC->store.Radio_Common_Header.m_ack = M_ACK_No_acknowledgement_requir;
  ...
  outC->store.Radio_Common_Header.m_version = M_VERSION_Previous_versions_acc;
  for (i = 0; i < 30; i++) {
  ...
  outC->toMoRC.BG_Common_Header.m_version = M_VERSION_Previous_versions_acc;
  ...
  outC->toMoRC.BG_Common_Header.bgPosition.motionDirection =
    unknownDirection_Obu_BasicTypes;
  outC->toMoRC.BG_Common_Header.BG_centerDetectionInaccuraccura.nominal = 0;
  outC->toMoRC.BG_Common_Header.BG_centerDetectionInaccuraccura.d_min = 0;
  outC->toMoRC.BG_Common_Header.BG_centerDetectionInaccuraccura.d_max = 0;
  outC->toMoRC.BG_Common_Header.q_nvlocacc = 0;
  outC->toMoRC.BG_Common_Header.noCoordinateSystemHasBeenAssign = kcg_true;
  ...
  outC->toMoRC.Radio_Common_Header.m_ack = M_ACK_No_acknowledgement_requir;
  ...
  outC->toMoRC.Radio_Common_Header.m_version = M_VERSION_Previous_versions_acc;
  for (i = 0; i < 30; i++) {
...
void keepP42_EVC_Support_Pkg(
  /* EVC_Support_Pkg::keepP42::fromTrack */ ReceivedMessage_T_Common_Types_ *fromTrack,
  outC_keepP42_EVC_Support_Pkg *outC)
{
  ...
  /* 3 */ if (_L3) {
    kcg_copy_ReceivedMessage_T_Comm(&outC->store, fromTrack);
  }
  else /* last_init_ck_store */ if (outC->init) {
    kcg_copy_ReceivedMessage_T_Comm(
      &outC->store,
      (ReceivedMessage_T_Common_Types_ *) &cNoTrackMsg_EVC_Support_Pkg);
  }
  outC->init = kcg_false;
  /* 4 */ if (outC->storeMsg) {
    kcg_copy_ReceivedMessage_T_Comm(&outC->toMoRC, &outC->store);
  }
  else {
    kcg_copy_ReceivedMessage_T_Comm(&outC->toMoRC, fromTrack);
  }
}
