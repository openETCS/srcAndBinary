  ...
  outC_registeringToTheRadioNetw_ *outC)
{
  ...
  _L185 = ((*sessionStatus).phase == sp_establishing_RCM_Session_Typ) |
    ((*sessionStatus).phase == sp_terminating_RCM_Session_Type) |
    ((*sessionStatus).phase == sp_maintaining_RCM_Session_Type);
  ...
  /* init_Register_SM */ if (outC->init) {
    ...
    outC->orderRegistrationDelayed = (triggerRegistrationOfMobile & (_L242 |
          (_L185 & (*sessionStatus).valid)) & br_1_guard_Register_SM_Reregist) |
      (!outC->resetDelayedOrderFlag & outC->orderRegistrationDelayed);
