/* $*************** KCG Version 6.1.3 (build i6) ****************
** Command: s2c613 -config C:/GITHUB/modeling/model/Scade/System/MEN_demonstrator/Simulation\kcg_s2c_config.txt
** Generation date: 2015-10-19T14:35:58
*************************************************************$ */

#include "kcg_consts.h"
#include "kcg_sensors.h"
#include "C_P005_extract_el_section_TM_lib_internal.h"

void C_P005_extract_el_section_reset_TM_lib_internal(
  outC_C_P005_extract_el_section_TM_lib_internal *outC)
{
}

/* TM_lib_internal::C_P005_extract_el_section */
void C_P005_extract_el_section_TM_lib_internal(
  /* TM_lib_internal::C_P005_extract_el_section::offset */kcg_int offset,
  /* TM_lib_internal::C_P005_extract_el_section::flat */P005_sections_array_flat_T_TM *flat,
  /* TM_lib_internal::C_P005_extract_el_section::modulo */kcg_int modulo,
  outC_C_P005_extract_el_section_TM_lib_internal *outC)
{
  kcg_copy_P005_sections_array_flat_T_TM(&outC->_L2, flat);
  outC->_L1 = offset;
  outC->_L3 = modulo;
  outC->_L5 = outC->_L1 + outC->_L3;
  if ((0 <= outC->_L5) & (outC->_L5 < 231)) {
    outC->_L6 = outC->_L2[outC->_L5];
  }
  else {
    outC->_L6 = 0;
  }
  outC->element = outC->_L6;
}

/* $*************** KCG Version 6.1.3 (build i6) ****************
** C_P005_extract_el_section_TM_lib_internal.c
** Generation date: 2015-10-19T14:35:58
*************************************************************$ */

